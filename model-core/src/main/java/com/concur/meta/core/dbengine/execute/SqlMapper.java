...
public class SqlMapper {
     ...
     * @return
     */
    public int insert(String sql, Object value, String keyProperty) {
        Class<?> parameterType = value != null ? value.getClass() : null;
        String msId = msUtils.insertDynamic(sql, parameterType, keyProperty);
    ...
    private class MSUtils {
         ...
         * @param keyProperty 主键名
         */
        private void newInsertMappedStatement(String msId, SqlSource sqlSource, SqlCommandType sqlCommandType,
                                              String keyProperty) {
            Builder builder = new Builder(configuration, msId, sqlSource, sqlCommandType)
                .resultMaps(new ArrayList<ResultMap>() {
                    {
                        add(new ResultMap.Builder(configuration, "defaultResultMap", Map.class, new ArrayList<ResultMapping>(0)).build());
                    }
        ...
        private String insertDynamic(String sql, Class<?> parameterType, String keyProperty) {
            String msId = newMsId(sql + parameterType, SqlCommandType.INSERT);
            if (hasMappedStatement(msId)) {
                return msId;
            }
            SqlSource sqlSource = languageDriver.createSqlSource(configuration, sql, parameterType);
            newInsertMappedStatement(msId, sqlSource, SqlCommandType.INSERT, keyProperty);
            return msId;
        }
